...
3. Kiểm tra lại toàn bộ cấu hình trực quan và nhấn nút **Create bucket** để hoàn tất khởi tạo.
   ...
   ![Khởi tạo S3 Bucket cho PharmaCare](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket1.jpg)
...
3. Cấu hình thông tin thư mục:
   ...
   * Nhấn nút **Create folder** để xác nhận.
   ...
   ![Tạo thư mục products trong S3](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket2.jpg)
...
4. Khi thư mục `products/` xuất hiện trong danh sách đối tượng với trạng thái sẵn sàng, click trực tiếp vào thư mục để tiến vào không gian lưu trữ bên trong.
   ...
   ![Danh sách đối tượng trong bucket](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket3.jpg)
...
6. Giữ nguyên hạng mục lưu trữ ở mức **Standard** và nhấn **Upload** để thực hiện truyền tải dữ liệu lên đám mây.
   ...
   ![Tải ảnh sản phẩm vào thư mục products](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket4.jpg)
...
2. **Bước 1 - Choose a plan:** Chọn gói **Free ($0/month)** được cung cấp cho mục đích học tập và phát triển hệ thống, tích hợp sẵn các bảo vệ DDoS (Always-on DDoS protection), chứng chỉ TLS miễn phí, Tiered caching, cùng định mức 1M requests / 100GB lưu lượng hàng tháng. Nhấn **Next**.
   ...
   ![Chọn gói CloudFront Free Plan](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket5.jpg)
...
3. **Bước 2 - Get started (Distribution options):**
   ...
   * Nhấn **Next**.
   ...
   ![Cấu hình thông tin cơ bản CloudFront](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket6.jpg)
...
4. **Bước 3 - Specify origin:**
   ...
   * Nhấn **Next**.
   ...
   ![Cấu hình Origin S3 cho CloudFront](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket7.jpg)
...
5. **Bước 4 - Enable security:**
   ...
   * Nhấn **Next**.
   ...
   ![Kích hoạt bảo mật WAF tích hợp](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket8.jpg)
...
6. **Bước 5 - Review and create:**
   ...
   * Nhấn nút **Create distribution** để kích hoạt tiến trình triển khai mạng lưới biên toàn cầu.
   ...
   ![Xác nhận và tạo CloudFront Distribution](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket9.jpg)
...
2. Ghi nhận các thông số mạng quan trọng để phục vụ bước cấu hình tích hợp cơ sở dữ liệu ứng dụng:
   ...
   * **ARN:** `arn:aws:cloudfront::392646748514:distribution/E105N1I4TWNVYQ`.
   ...
   ![Trạng thái triển khai CloudFront thành công](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket10.jpg)
...
2. Chỉnh sửa tệp logic xử lý migration (`index.mjs`) để tự động hóa việc cập nhật tên miền CloudFront mới vào cột `image_url` của bảng `products` trong cơ sở dữ liệu Amazon RDS.
   * **Đoạn mã JavaScript xử lý cập nhật (`index.mjs`):**
   ...
   ![Chỉnh sửa mã nguồn migration trong VS Code](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket11.jpg)
...
5. Trong giao diện **Code source**, nhấn **Upload from** → **.zip file**, sau đó tải lên file `function.zip` vừa được đóng gói.
   ...
   ![Cập nhật và triển khai mã nguồn lên AWS Lambda](/NguyenThiThuyHien_Workshop/images/5-Workshop/5.4-lambda-backend/5.4.2-s3-product-images/s3bucket12.jpg)
